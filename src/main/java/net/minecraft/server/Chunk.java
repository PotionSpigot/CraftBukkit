...
public class Chunk
{
  private static final org.apache.logging.log4j.Logger t = ;
  ...
  {
    if ((!this.world.paperSpigotConfig.cacheChunkMaps) || (!groundUpContinuous) || ((primaryBitMask != 0) && (primaryBitMask != 65535))) {
    ...
    for (int i = 0; i < this.sections.length; i++) {
      ...
      if (section == null) {
        if ((this.emptySectionBits & 1 << i) == 0) {
          isDirty = true;
          this.emptySectionBits |= 1 << i;
        }
      }
      else if ((this.emptySectionBits & 1 << i) == 1) {
        isDirty = true;
        this.emptySectionBits &= (1 << i ^ 0xFFFFFFFF);
    ...
    if (isDirty) {
      this.chunkMap17 = null;
      this.chunkMap18 = null;
    }
    
    if (version < 24) {
      if (this.chunkMap17 == null) {
        this.chunkMap17 = PacketPlayOutMapChunk.a(this, true, 65535, version);
      }
      
      return this.chunkMap17;
    }
    if (this.chunkMap18 == null) {
      this.chunkMap18 = PacketPlayOutMapChunk.a(this, true, 65535, version);
    }
    
    return this.chunkMap18;
  }
  ...
  public boolean areNeighborsLoaded(int radius) { switch (radius) {
    case 2: 
  ...
  public Chunk(World world, int i, int j)
  {
    ...
    this.tileEntities = new java.util.HashMap();
    this.x = 4096;
    ...
    for (int k = 0; k < this.entitySlices.length; k++) {
      this.entitySlices[k] = new org.bukkit.craftbukkit.v1_7_R4.util.UnsafeList();
    }
    
    Arrays.fill(this.b, 64537);
      ...
      for (int i1 = 0; i1 < 16; i1++) {
        for (int j1 = 0; j1 < k; j1++) {
          ...
          if ((block != null) && (block.getMaterial() != Material.AIR)) {
            ...
            this.sections[k1].setTypeId(l, j1 & 0xF, i1, block);
          }
        }
      }
    }
      ...
      for (int i1 = 0; i1 < 16; i1++) {
        for (int j1 = 0; j1 < k; j1++) {
          ...
          if ((block != null) && (block != Blocks.AIR)) {
            ...
            this.sections[l1].setTypeId(l, j1 & 0xF, i1, block);
            this.sections[l1].setData(l, j1 & 0xF, i1, checkData(block, abyte[k1]));
          }
        }
      }
    }
          ...
          do
          {
            ...
            if (l > 0) {
              ...
              if (chunksection != null) {
                chunksection.setSkyLight(j, i1 & 0xF, k, l);
  ...
  private void c(int i, int j, int k, int l) {
    if ((l > k) && (this.world.areChunksLoaded(i, 0, j, 16))) {
      for (int i1 = k; i1 < l; i1++) {
        this.world.updateLight(EnumSkyBlock.SKY, i, i1, j);
      }
      ...
      if (!this.world.worldProvider.g)
      {
        ...
        if (i1 < l) {
          for (int l1 = i1; l1 < l; l1++) {
            ...
            if (chunksection != null) {
              chunksection.setSkyLight(i, l1 & 0xF, k, 15);
        ...
        for (int l1 = l; l1 < i1; l1++) {
          ChunkSection chunksection = this.sections[(l1 >> 4)];
          if (chunksection != null) {
            chunksection.setSkyLight(i, l1 & 0xF, k, 0);
            this.world.m((this.locX << 4) + i, l1, (this.locZ << 4) + k);
          }
        }
        ...
        while ((i1 > 0) && (l1 > 0)) {
          ...
          if (chunksection1 != null) {
            chunksection1.setSkyLight(i, i1 & 0xF, k, l1);
          }
        }
      }
  ...
  public boolean a(int i, int j, int k, Block block, int l)
  {
    ...
    chunksection.setData(i, j & 0xF, k, checkData(block, l));
    if (flag) {
      initLighting();
    } else {
      int j2 = block.k();
  ...
  public boolean a(int i, int j, int k, int l)
  {
    ...
    int i1 = chunksection.getData(i, j & 0xF, k);
    
    if (i1 == l) {
      return false;
    }
    this.n = true;
    Block block = chunksection.getTypeId(i, j & 0xF, k);
    chunksection.setData(i, j & 0xF, k, checkData(block, l));
    if ((block instanceof IContainer)) {
      TileEntity tileentity = e(i, j, k);
  ...
  public int getBrightness(EnumSkyBlock enumskyblock, int i, int j, int k)
  {
    ...
    return enumskyblock == EnumSkyBlock.BLOCK ? chunksection.getEmittedLight(i, j & 0xF, k) : enumskyblock == EnumSkyBlock.SKY ? chunksection.getSkyLight(i, j & 0xF, k) : this.world.worldProvider.g ? 0 : chunksection == null ? 0 : d(i, j, k) ? enumskyblock.c : enumskyblock.c;
  }
  ...
  public int b(int i, int j, int k, int l) {
    ...
    int i1 = this.world.worldProvider.g ? 0 : chunksection.getSkyLight(i, j & 0xF, k);
    ...
    i1 -= l;
    int j1 = chunksection.getEmittedLight(i, j & 0xF, k);
  ...
  public void a(Entity entity)
  {
    ...
    {
      org.bukkit.Bukkit.getLogger().warning("Wrong location for " + entity + " in world '" + this.world.getWorld().getName() + "'!");
      

      org.bukkit.Bukkit.getLogger().warning("Entity is at " + entity.locX + "," + entity.locZ + " (chunk " + i + "," + j + ") but was stored in chunk " + this.locX + "," + this.locZ);
    }
  ...
  public TileEntity e(int i, int j, int k) {
    ...
    if (tileentity == null) {
      ...
      tileentity = ((IContainer)block).a(this.world, getData(i, j, k));
  ...
  public void a(int i, int j, int k, TileEntity tileentity) {
    ...
    if ((getType(i, j, k) instanceof IContainer)) {
      ...
      (CraftMagicNumbers.getMaterial(getType(i, j, k)) != org.bukkit.Material.MOB_SPAWNER)) {
      this.tileEntities.remove(chunkposition);
    }
    else
    {
      System.out.println("Attempted to place a tile entity (" + tileentity + ") at " + tileentity.x + "," + tileentity.y + "," + tileentity.z + " (" + 
        CraftMagicNumbers.getMaterial(getType(i, j, k)) + ") where there was no entity tile!");
  ...
  public void removeEntities() {
    ...
    while (iterator.hasNext()) {
      ...
      {
        for (org.bukkit.entity.HumanEntity h : new ArrayList(((IInventory)tileentity).getViewers()))
        {
    ...
    for (int i = 0; i < this.entitySlices.length; i++)
    {
      Object iter = this.entitySlices[i].iterator();
      while (((Iterator)iter).hasNext()) {
        Entity entity = (Entity)((Iterator)iter).next();
        
        if ((entity instanceof IInventory))
        {
          for (org.bukkit.entity.HumanEntity h : new ArrayList(((IInventory)entity).getViewers()))
          {
            if ((h instanceof CraftHumanEntity))
            {
              ((CraftHumanEntity)h).getHandle().closeInventory();
  ...
  public boolean a(boolean flag) {
    ...
    } else if ((this.o) && (this.world.getTime() >= this.lastSaved + MinecraftServer.getServer().autosavePeriod * 4)) {
      return true;
    }
  ...
  public void b(boolean flag) {
    if ((this.w) && (!this.world.worldProvider.g) && (!flag)) {
      recheckGaps(this.world.isStatic);
    }
  ...
  private boolean f(int i, int j) {
    ...
    for (int l = k + 16 - 1; (l > 63) || ((l > 0) && (!flag1)); l--) {
      int i1 = b(i, l, j);
