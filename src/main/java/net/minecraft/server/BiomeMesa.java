...
public class BiomeMesa extends BiomeBase
{
  ...
  public void a(World world, Random random, Block[] ablock, byte[] abyte, int i, int j, double d0) {
    ...
    if (this.aH) {
      int k = (i & 0xFFFFFFF0) + (j & 0xF);
      int l = (j & 0xFFFFFFF0) + (i & 0xF);
      double d2 = Math.min(Math.abs(d0), this.aE.a(k * 0.25D, l * 0.25D));
      
      if (d2 > 0.0D) {
        double d3 = 0.001953125D;
        double d4 = Math.abs(this.aF.a(k * d3, l * d3));
  ...
  private void a(long i) {
    this.aC = new byte[64];
    java.util.Arrays.fill(this.aC, (byte)16);
    ...
    for (int l = 0; l < k; l++) {
      ...
      int j1 = random.nextInt(64);
      
      for (int k1 = 0; (j1 + k1 < 64) && (k1 < i1); k1++) {
        this.aC[(j1 + k1)] = 12;
      }
    }
  ...
  private byte d(int i, int j, int k) {
    int l = (int)Math.round(this.aG.a(i * 1.0D / 512.0D, i * 1.0D / 512.0D) * 2.0D);
