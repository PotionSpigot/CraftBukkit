...
public class ItemSkull
  extends Item
{
  private static final String[] b = { "skeleton", "wither", "zombie", "char", "creeper" };
  public static final String[] a = { "skeleton", "wither", "zombie", "steve", "creeper" };
  ...
  public boolean interactWith(ItemStack itemstack, EntityHuman entityhuman, World world, int i, int j, int k, int l, float f, float f1, float f2) {
    if (l == 0)
      return false;
    if (!world.getType(i, j, k).getMaterial().isBuildable()) {
      return false;
    }
    if (l == 1) {
      j++;
    ...
    if (!world.isStatic)
    {
      ...
      if (l == 1) {
        i1 = MathHelper.floor(entityhuman.yaw * 16.0F / 360.0F + 0.5D) & 0xF;
      }
