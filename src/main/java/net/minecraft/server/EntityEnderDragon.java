...
public class EntityEnderDragon extends EntityInsentient implements IComplex, IMonster
{
  ...
  private Explosion explosionSource = new Explosion(null, this, NaN.0D, NaN.0D, NaN.0D, NaN.0F);
  ...
  public double[] b(int i, float f) {
    ...
    f = 1.0F - f;
    int j = this.bo - i * 1 & 0x3F;
    int k = this.bo - i * 1 - 1 & 0x3F;
    ...
    d1 = this.bn[k][1] - d0;
    adouble[1] = (d0 + d1 * f);
    adouble[2] = (this.bn[j][2] + (this.bn[k][2] - this.bn[j][2]) * f);
    return adouble;
  }
  ...
  public void e()
  {
    if (this.world.isStatic) {
      float f = MathHelper.cos(this.by * 3.1415927F * 2.0F);
      float f1 = MathHelper.cos(this.bx * 3.1415927F * 2.0F);
      if ((f1 <= -0.3F) && (f >= -0.3F)) {
    ...
    if (getHealth() <= 0.0F) {
      ...
      if (this.world.isStatic) {
        ...
        Vec3D vec3d1 = Vec3D.a(MathHelper.sin(this.yaw * 3.1415927F / 180.0F), this.motY, -MathHelper.cos(this.yaw * 3.1415927F / 180.0F)).a();
      ...
      for (int j = 0; j < 3; j++) {
        ...
        float f14 = this.yaw * 3.1415927F / 180.0F + b(adouble2[0] - adouble[0]) * 3.1415927F / 180.0F * 1.0F;
  ...
  private void bP() {
    if (this.bC != null) {
      if (this.bC.dead) {
        if (!this.world.isStatic) {
          org.bukkit.craftbukkit.v1_7_R4.event.CraftEventFactory.entityDamage = this.bC;
          a(this.bq, DamageSource.explosion((Explosion)null), 10.0F);
      ...
      {
        EntityRegainHealthEvent event = new EntityRegainHealthEvent(getBukkitEntity(), 1.0D, org.bukkit.event.entity.EntityRegainHealthEvent.RegainReason.ENDER_CRYSTAL);
  ...
  private void bQ() {
    ...
    if ((this.random.nextInt(2) == 0) && (!this.world.players.isEmpty()))
    {
      ...
      EntityTargetEvent event = new EntityTargetEvent(getBukkitEntity(), target.getBukkitEntity(), org.bukkit.event.entity.EntityTargetEvent.TargetReason.RANDOM_TARGET);
      this.world.getServer().getPluginManager().callEvent(event);
  ...
  public boolean a(EntityComplexPart entitycomplexpart, DamageSource damagesource, float f) {
    ...
    this.h = (this.locX + f2 * 5.0F + (this.random.nextFloat() - 0.5F) * 2.0F);
    this.i = (this.locY + this.random.nextFloat() * 3.0F + 1.0D);
    this.bm = (this.locZ - f3 * 5.0F + (this.random.nextFloat() - 0.5F) * 2.0F);
    this.bD = null;
  ...
  protected void aF() {
    if (this.dead) return;
    this.bB += 1;
    float f2; if ((this.bB >= 180) && (this.bB <= 200)) {
      float f = (this.random.nextFloat() - 0.5F) * 8.0F;
      float f1 = (this.random.nextFloat() - 0.5F) * 4.0F;
      f2 = (this.random.nextFloat() - 0.5F) * 8.0F;
      
      this.world.addParticle("hugeexplosion", this.locX + f, this.locY + 2.0D + f1, this.locZ + f2, 0.0D, 0.0D, 0.0D);
    }
    ...
    if (!this.world.isStatic) {
      ...
      if (this.bB == 1)
      {
        ...
        for (EntityPlayer player : this.world.players) {
          ...
          if ((this.world.spigotConfig.dragonDeathSoundRadius <= 0) || (distanceSquared <= this.world.spigotConfig.dragonDeathSoundRadius * this.world.spigotConfig.dragonDeathSoundRadius)) {
    ...
    if ((this.bB == 200) && (!this.world.isStatic)) {
      ...
      b(MathHelper.floor(this.locX), MathHelper.floor(this.locZ));
      die();
    }
  }
  ...
  private void b(int i, int j) {
    ...
    if (!event.isCancelled()) {
      ...
      for (BlockState state : event.getBlocks()) {
        packet = new PacketPlayOutBlockChange(state.getX(), state.getY(), state.getZ(), this.world);
