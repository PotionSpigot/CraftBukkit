  ...
  public static EntityPlayer[] getPlayers(ICommandListener icommandlistener, String s)
  {
    ...
    if (matcher.matches()) {
      ...
      if ((!s1.equals("p")) && (!s1.equals("a"))) {
        if (s1.equals("r")) {
          List list = MinecraftServer.getServer().getPlayerList().a(chunkcoordinates, i, j, 0, j1, k, l, map1, s2, s3, world);
      ...
      List list = MinecraftServer.getServer().getPlayerList().a(chunkcoordinates, i, j, i1, j1, k, l, map1, s2, s3, world);
      return list.isEmpty() ? new EntityPlayer[0] : (EntityPlayer[])list.toArray(new EntityPlayer[list.size()]);
    }
